...
import { stockifyLot } from '../services/assemblyService';
...
type Props = {
    navigation: NativeStackNavigationProp<RootStackParamList, 'stockify'>;
};
...
export default function Stockify({ navigation }: Props) {
    // ── Read stepId from route params — correct way for stack screens ──────────
    const route = useRoute<RouteProp<RootStackParamList, 'stockify'>>();
    ...
    const handleSubmit = async () => {
        ...
        try {
            ...
            const lot = await stockifyLot(payload);

            play();

            Alert.alert(
                'Stockify',
                `Lot ${lot?.lot_number ?? ''} Stockify successfully. ✅`,
