...
import { fetchCategories, fetchCities } from '../services/contactService';
import { Contact, FilterState, ContactCategory, City } from '../types/contact';
...
import { fetchVendors } from '../services/assemblyService';

type Props = {
  navigation: NativeStackNavigationProp<RootStackParamList, 'Contacts'>;
};

const VendorsScreen: React.FC<Props> = ({ navigation }) => {
  ...
  const {
    ...
    queryFn: async ({ pageParam }: { pageParam: number }) => {
      ...
      const response = await fetchVendors(filterParams as any);
      return response;
    },
  ...
  const handleContactPress = useCallback(
    (contact: Contact) => {
      navigation.navigate('lotLedger', { contact: contact });
    },
    [navigation]
  );
      ...
      >
        ...
        <View style={styles.contactRight}>
          <View style={styles.balanceContainer}>
            <Text style={styles.balanceLabel}>Active Lots</Text>
            <Text style={[styles.balanceAmount, { color: colors.gray700 }]}>
             {item.lots}
            </Text>
          </View>
  ...
  return (
    <SafeAreaView style={styles.container} edges={['top', 'left', 'right']}>
      ...
      {/* Header */}
      <Header title='Vendors' navigation={null}></Header>
      ...
      {/* Bottom Navigation */}
      <AssemblyNavigation activeRoute='vendors' />
    </SafeAreaView>
  );
};
