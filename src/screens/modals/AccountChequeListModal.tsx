...
import { ContactChequeListItem, Cheque } from '../../types/cheques';
import { getAccountChequeList, updateCheque, recordInstallment } from '../../services/cheques';
...
// ─── Account-ledger cheque drill-down — the account cheque summary only ever
// surfaces two cards ("Issued Cheques" / "Installment Cheques"), so this only
// wires up the same actions ChequeListScreen already uses for those two
// statuses (Clear / Installment / Bounce / Exchange-to-Cash), via the shared
// utils/chequeActions module. ──

/** Width (px) of each swipe action button — matches ChequeListScreen. */
const ACTION_BUTTON_WIDTH = 64;

/** The only two cheque_status values the account-cheque-summary cards use. */
export type AccountChequeStatus = 'issued' | 'installment';

// ─── Props ────────────────────────────────────────────────────────────────────

interface AccountChequeListModalProps {
    visible: boolean;
    accountId: number;
...
const AccountChequeListModal: React.FC<AccountChequeListModalProps> = ({
    visible, accountId, status, title, currency, onClose, onActionSuccess,
}) => {
    ...
    // `status` (the API's `cheque_status`) is used directly as the route type — "issued" or "installment".
    const { data: cheques = [], isLoading, isError, isRefetching, refetch } = useQuery({
        queryKey: ['accountChequeList', accountId, status],
        queryFn: () => getAccountChequeList(accountId, status),
    ...
    // Only Clear/Bounce/Installment/Exchange are reachable for issued & installment
    // cheques (see getChequeSwipeActions) — no Return action applies here.
    ...
    const actions = useMemo(
        () => getChequeSwipeActions(status, {
            ...
            onReturn: handleClear, // unreachable for issued/installment — kept only to satisfy the shared handler shape
            onExchange: handleExchange,
        }),
        [status, handleClear, handleBounce, handleInstallment, handleExchange],
    );
...
export default AccountChequeListModal;
