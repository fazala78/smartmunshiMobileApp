...
const PurchaseCheckoutModal: React.FC<PurchaseCheckoutModalProps> = ({
    ...
    const checkOut = async () => {
        ...
        try {
            ...
            const invoice = await createPurchase(submitPayload as any);
            play();
            onConfirm(invoice);
        ...
        <Modal visible={visible} animationType="slide" transparent onRequestClose={onDismiss}>
            <View style={styles.overlay}>
                {/*
                  ...
                  │  Wraps the entire bottom sheet so the footer (swipe button) │
                  │  is pushed up when the software keyboard appears.           │
                  │                                                             │
                  │  • behavior="padding"  works on both iOS & Android.         │
                  │  • keyboardVerticalOffset accounts for the status bar /     │
                  │    any extra space at the top of the modal.                 │
                  └────────────────────────────────────────────────────────────┘
                */}
                <KeyboardAvoidingView
                    ...
                    <View style={styles.container}>
                        ...
                        >
                            ...
                            <InputField
                                ...
                                type="text"
                                value={String(payload?.invoice_number || '')}
                                onChangeText={(value) => {
                                    clearErrors();
                                    setPayload((prev) => {
                                        if (!prev) return prev;
                        ...
                        <View style={styles.footer}>
                            ...
                            <SwipeButton
                                ...
                                titleFontSize={14}
                                height={52}          // ← FIX 2: reduced from 64 → 52
...
const styles = StyleSheet.create({
  ...
  contactChipText: { textTransform:'capitalize', fontSize: 15, fontWeight: '600', color: colors.gray600, maxWidth: 500 },
    ...
    footer: {
        padding: 16,               // reduced from 20 → 16 to keep footer compact
        backgroundColor: colors.white,
        borderTopWidth: 1,
