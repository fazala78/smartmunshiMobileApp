...
import { nextStep } from '../services/assemblyService';
import useCurrency from '../utils/currency';
import ProductionBillingStep from '../components/assembly/ProductionBillingStep';
...
type Props = {
    navigation: NativeStackNavigationProp<RootStackParamList, 'nextProcess'>;
};
...
function validateScreen(screenIndex: number, data: LotFormData): string | null {
    switch (screenIndex) {
        ...
        case 3:
            // HandOverStep — manufacturer + process required
            if (!data.manufacturer)
                return 'Please select a manufacturer before continuing.';
            if (!data.process)
                return 'Please select a process before continuing.';
            return null;
...
export default function NextProcessScreen({ navigation }: Props) {
    // ── Read stepId from route params — correct way for stack screens ──────────
    const route = useRoute<RouteProp<RootStackParamList, 'nextProcess'>>();
    ...
    const handleSubmit = async () => {
        ...
        try {
            const payload: any = {
                ...formData,
                step_id: stepId,                              // pass the originating step id
            ...
            Alert.alert(
                'Lot Created',
                `Lot ${lot?.lot_number ?? ''} created successfully. ✅`,
    ...
    const renderStep = () => {
        switch (currentStep) {
            ...
            case 5:
                return <LotSummaryStep formData={formData} totalSteps={TOTAL_STEPS} />;
            default:
                return null;
        }
    };
