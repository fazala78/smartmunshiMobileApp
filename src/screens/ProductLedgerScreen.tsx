...
import { Product, ProductTransaction } from '../types/Product';
import StockAdjustment from '../components/StockAdjustment';
import { getProductTransactions, getProductTransactionTypes } from '../services/ProductService';
...
const ProductLedgerScreen: React.FC<Props> = ({ route }) => {
    ...
    const { data: productTransactionType = [] } = useQuery({
        queryKey: ['productTransactionTypes'],
        queryFn: getProductTransactionTypes,
        staleTime: 5 * 60 * 1000,
    });
    ...
    const {
        ...
        queryKey: ['productLedger', route.params.product.id, filters],
        queryFn: async ({ pageParam }: { pageParam: number }) => {
            const response = await getProductTransactions(route.params.product.id, {
                page: pageParam,
                search: {
    ...
    const transactionsWithBalance = useMemo(() => {
        ...
        let running = route.params.product.balance;
        return all.map((tx) => {
            const balance = running;
            running = running - (tx.debit || 0) + (tx.credit || 0);
            return { ...tx, balance };
        });
    }, [data, route.params.product.balance]);

    // ── Helpers ───────────────────────────────────────────────────────────────
    const balanceColor = route.params.product.balance < 0 ? colors.error : colors.primary;
    ...
    const handleAdjustment = useCallback((value: string) => {
        setAdjustment(value);
        setPaymentModal(true);
    }, []);

    const handleSelectedTransaction = useCallback((transaction: ProductTransaction) => {
        ...
        <TouchableOpacity onPress={() => handleSelectedTransaction(item)} activeOpacity={0.8}>
            <View style={styles.transactionItem}>
                ...
                <View style={styles.transactionDescriptionContainer}>
                    ...
                    {item.lot && (
                        <Text style={styles.transactionDate}>Lot# {item.lot}</Text>
                    )}
                    {item.contact && (
                        <Text style={styles.transactionDate}>Contact: {item.contact}</Text>
                    )}
        ...
        <>
            <SafeAreaView style={styles.safeArea}>
                ...
                <View style={styles.header}>
                    ...
                    <Text style={styles.headerTitle}>Product Ledger</Text>
                </View>
                ...
                <View style={styles.profileSection}>
                    ...
                    <View style={styles.contactInfo}>
                        <Text style={styles.contactName}>{route.params.product.name}</Text>
                    </View>
                    <View style={styles.balanceContainer}>
                        <Text style={styles.balanceLabel}>Current Balance</Text>
                        <Text style={[styles.balanceAmountLarge, { color: balanceColor }]}>
                            {route.params.product.balance} 
                            <Text style={{ fontSize: 14, color: balanceColor,textTransform:'capitalize' }}>
                                 {' '}{route.params.product.unit}
                            </Text>
                ...
                <View style={styles.bottomActions}>
                    <TouchableOpacity style={styles.creditButton} activeOpacity={0.8} onPress={() => handleAdjustment('-')}>
                        <Text style={styles.buttonIcon}>⊖</Text>
                        <Text style={styles.buttonText}>Remove Stock</Text>
                    </TouchableOpacity>
                    <TouchableOpacity style={styles.receiveButton} activeOpacity={0.8} onPress={() => handleAdjustment('+')}>
                        <Text style={styles.buttonIcon}>⊕</Text>
                ...
                >
                    ...
                    <View style={styles.transactionTypeContainer}>
                        ...
                        <View style={styles.filterButtonsContainer}>
                            {productTransactionType.map((item: any) => (
...
export default ProductLedgerScreen;

// ─── Styles ───────────────────────────────────────────────────────────────────
const styles = StyleSheet.create({
    safeArea: { flex: 1, backgroundColor: colors.white },
    overlay: { flex: 1, backgroundColor: colors.backgroundOverlay, justifyContent: 'flex-end' },
    container: { backgroundColor: colors.white, borderTopLeftRadius: 24, borderTopRightRadius: 24, height: '90%', overflow: 'hidden' },
