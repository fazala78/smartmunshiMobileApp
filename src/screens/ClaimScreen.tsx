...
import { deductClaim } from '../services/assemblyService';
import useCurrency from '../utils/currency';
import ClaimStep from '../components/assembly/ClaimStep';
...
function validateScreen(data: FormData): string | null {
    if (data.consum_products) {
        for (const cp of data.consum_products) {
            const available = cp.supplied_qty - cp.used_qty;
            const qty = cp.quantity ?? 0;

            if (qty < 0) {
                return `"${cp.name}": quantity cannot be negative.`;
            }

            if (qty > available) {
                return `"${cp.name}": entered ${qty} ${cp.unit} but only ${available} ${cp.unit} available (${cp.supplied_qty} supplied − ${cp.used_qty} used).`;
            }
        }
    }
...
export default function ClaimScreen({ navigation }: Props) {
    ...
    const goNext = () => {
        const error = validateScreen(formData);
    ...
    const handleSubmit = async () => {
        ...
        try {
            ...
            const lot = await deductClaim(payload);

            play();

            Alert.alert(
                'Claim Deducted',
                `Claim deducted from Lot ${lot?.lot_number ?? ''} successfully. ✅`,
        ...
        switch (currentStep) {
            case 1:
                ...
                return (
                    <ClaimStep
                        data={formData}
                        setFormData={setFormData} step={step} />
                );
