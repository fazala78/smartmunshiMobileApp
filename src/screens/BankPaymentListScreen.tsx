...
import { getBankPaymentTransactions, deletePayment, getPaymentMethods } from '../services/bankPaymentService';
...
type Props = {
    navigation: NativeStackNavigationProp<RootStackParamList, 'bankPayments'>;
    route:      RouteProp<RootStackParamList, 'bankPayments'>;
};
...
const BankPaymentListScreen: React.FC<Props> = ({ navigation, route }) => {

    const { item } = route.params ?? {};

    const [showFilterModal,     setShowFilterModal]     = useState(false);
    const [filters,             setFilters]             = useState<any>({ accounts: [], payment_method: '', searchQuery: '', date: toDateString(new Date()) });
    ...
    const {
        ...
        queryFn:  ({ pageParam = 1 }) =>
            getBankPaymentTransactions(item?.route, { ...filters, page: pageParam }),
        initialPageParam: 1,
    ...
    const handleEdit = useCallback((_tx: PaymentListing) => {
        // navigation.navigate('EditBankPayment', { id: tx.id });
    }, []);
    ...
    const handleResetFilters = useCallback(() => {
        const reset = { accounts: [], payment_method: '', searchQuery: '', date: filters.date };
    ...
    return (
        <SafeAreaView style={styles.container} edges={['top', 'left', 'right']}>
            ...
            <Header title={item?.label ?? 'Bank Payments'} navigation={navigation} />
...
export default BankPaymentListScreen;
